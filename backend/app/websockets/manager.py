...
class ConnectionManager:
    ...
    async def send_event(
        ...
        """Publish a typed JSON event to a specific session via Redis."""
        message = {
    ...
    async def broadcast_to_user(
        ...
        """Publish an event to all sessions associated with a user email via Redis.
        Note: If this backend replica doesn't have the session, it won't push directly,
        but since it goes via Redis, the replica that DOES have the session will push it.
        We must publish to all known session_ids across the cluster.
        Wait, we only know about our local sessions for the user! 
        To fix this, we should broadcast the user email in the message and let the listener resolve it.
        """
        message = {
            "user_email": email,
            "event_type": event_type,
