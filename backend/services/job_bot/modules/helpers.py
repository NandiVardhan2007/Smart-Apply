...
def get_default_temp_profile(user_id: str = None) -> str:
    # Thanks to https://github.com/vinodbavage31 for suggestion!
    # NOTE: Returns only the PATH — open_chrome.py adds the --user-data-dir= prefix
    # NOW: Creates user-specific profiles to prevent session bleeding between users
    home = pathlib.Path.home()
    
    # Create user-specific profile directory if user_id is provided
    profile_suffix = f"-{user_id}" if user_id else ""
    
    if sys.platform.startswith('win'):
        return f"C:\\temp\\auto-job-apply-profile{profile_suffix}"
    elif sys.platform.startswith('linux'):
        return str(home / f".auto-job-apply-profile{profile_suffix}")
    return str(home / "Library" / "Application Support" / "Google" / "Chrome" / f"auto-job-apply-profile{profile_suffix}")
