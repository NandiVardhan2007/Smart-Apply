...
from modules.external_form_filler import load_profile, fill_external_form, set_gemini_key
...
if run_in_background == True:
    ...
    # Note: run_non_stop is NOT overridden here so the value from settings.py is respected
...
# Load profile for external form auto-filler
load_profile(
    first_name=first_name, middle_name=middle_name, last_name=last_name,
    phone_number=phone_number, current_city=current_city, street=street,
    state=state, zipcode=zipcode, country=country,
    email=username,
    linkedin_profile=linkedIn, website=website,
    desired_salary=desired_salary, current_ctc=current_ctc,
    notice_period=notice_period, years_of_experience=years_of_experience,
    cover_letter=cover_letter, linkedin_headline=linkedin_headline,
    linkedin_summary=linkedin_summary, user_information_all=user_information_all,
    default_resume_path=default_resume_path,
    require_visa=require_visa, us_citizenship=us_citizenship,
    recent_employer=recent_employer, confidence_level=confidence_level,
)
# Pass Gemini key to external form filler
set_gemini_key(llm_api_key)
...
def login_LN() -> None:
    '''
    Authenticate with LinkedIn using saved browser session cookies.
    Cookies are set up once via the /cookie-setup page in the dashboard.
    This function does NOT use username/password — LinkedIn blocks headless
    form logins from server IPs. Cookie-based auth is the only reliable method.
    '''
    cookie_path = _cookie_path()
    import os
    if not os.path.exists(cookie_path):
        print_lg("=" * 60)
        print_lg("AUTHENTICATION REQUIRED")
        print_lg("No LinkedIn session cookies found.")
        print_lg("")
        print_lg("To set up authentication:")
        print_lg("  1. Go to your SmartApply dashboard")
        print_lg("  2. Click 'Setup LinkedIn Session' (or visit /cookie-setup.html)")
        print_lg("  3. Follow the 3-step instructions to export cookies from your browser")
        print_lg("  4. Paste the cookies and click Save")
        print_lg("  5. Restart the bot")
        print_lg("=" * 60)
    ...
    # Cookies loaded but session is invalid (expired/revoked)
    print_lg("=" * 60)
    print_lg("SESSION EXPIRED OR INVALID")
    print_lg("Your saved LinkedIn cookies are no longer valid.")
    print_lg("LinkedIn session cookies expire or get revoked when:")
    print_lg("  - You logged out of LinkedIn in your browser")
    print_lg("  - LinkedIn expired the session (usually after ~1 year)")
    print_lg("  - Your account triggered a security check")
    print_lg("")
    print_lg("Fix: Go to /cookie-setup.html in your dashboard and re-export fresh cookies.")
    print_lg("=" * 60)
    raise RuntimeError("LinkedIn session expired — re-export cookies from your browser.")
#>
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                print_lg("All filter retries failed due to stale elements, continuing anyway...")
        except Exception as e:
    ...
    try:
        ...
        for word in bad_words:
            if word.lower() in jobDescriptionLow:
                ...
                skip = True
                break
        # Smarter security clearance check — avoids false positives on Indian finance terms
        # e.g. "GST clearance", "payment clearance", "security deposit" should NOT trigger skip
        if not skip and security_clearance == False:
            clearance_phrases = [
                'security clearance', 'polygraph', 'secret clearance',
                'top secret', 'ts/sci', 'classified clearance',
                'dod clearance', 'government clearance'
            ]
            if any(phrase in jobDescriptionLow for phrase in clearance_phrases):
...
def answer_common_questions(label: str, answer: str) -> str:
    if 'sponsorship' in label or 'visa' in label: 
        answer = require_visa
    elif 'year' in label or ('experience' in label and 'month' not in label):
        # Dropdown like "0 year / 1 year / 2 years …"
        answer = str(years_of_experience)
    elif 'month' in label:
        # Dropdown like "0 month / 1 month … 11 months" — additional months beyond years
        answer = "0"
    elif 'notice' in label:
        answer = str(notice_period)
    elif 'ctc' in label or 'compensation' in label or 'salary' in label or 'pay' in label:
...
def answer_questions(modal: WebElement, questions_list: set, work_location: str, job_description: str | None = None ) -> set:
    ...
    for Question in all_questions:
        ...
        if select:
            ...
            if overwrite_previous_answers or selected_option == "Select an option":
                ...
                # Add location handling
                    ...
                    # "Willing/comfortable to work in X?" is a Yes/No question, not a geographic dropdown
                    yes_no_labels = ['willing', 'comfortable', 'open to', 'able to', 'relocat', 'commut']
                    is_yn_question = any(kw in label for kw in yes_no_labels)
                    yn_options = [o for o in optionsText if o.lower() not in ('select an option', '')]
                    is_yn_options = yn_options and all(
                        any(kw in o.lower() for kw in ['yes', 'no', 'agree', 'disagree', 'i do', "i don't"])
                        for o in yn_options
                    )
                    if is_yn_question or is_yn_options:
                        answer = 'Yes'
        ...
        do_actions = False  # reset for each question block
...
def external_apply(pagination_element: WebElement, job_id: str, job_link: str, resume: str, date_listed, application_link: str, screenshot_name: str) -> tuple[bool, str, int]:
    ...
    try:
        ...
        try:
            fill_external_form(driver, application_link)
        except Exception as fill_err:
            print_lg(f"External form filler encountered an error: {fill_err}")
...
def failed_job(job_id: str, job_link: str, resume: str, date_listed, error: str, exception: Exception, application_link: str, screenshot_name: str) -> None:
    ...
    try:
        ...
        print_lg("Failed to update failed jobs list - file may be in use")
...
def submitted_jobs(job_id: str, title: str, company: str, work_location: str, work_style: str, description: str, experience_required: int | Literal['Unknown', 'Error in extraction'], 
    ...
    try:
        ...
        print_lg("Failed to update applied jobs list - file may be in use")
        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                    if keep_screen_awake: pass  # screen awake via OS, not pyautogui
                    ...
                    if try_xp(driver, ".//button[contains(@class,'jobs-apply-button') and contains(@class, 'artdeco-button--3') and contains(@aria-label, 'Easy')]"):
                        try: 
                            try:
                                ...
                                while next_button:
                                    ...
                                    if next_counter >= 15: 
                                        if pause_at_failed_question:
                                            ...
                                            print_lg("Couldn't answer one or more questions. Skipping after manual review...")
                                            next_counter = 1
                                            continue
                                ...
                                cur_pause_before_submit = False  # auto-submit, no popup
                                ...
                                if wait_span_click(driver, "Submit application", 2, scrollTop=True): 
                                    ...
                                    # auto-assume submitted if submit button not found
                                    date_applied = datetime.now()
                    ...
                    # ANTI-DETECTION: take a human-like break every 10 Easy Apply submissions
                    if easy_applied_count > 0 and easy_applied_count % 10 == 0:
                        cooldown = randint(180, 360)  # Random 3–6 minute pause
                        print_lg(f"\n⏸  Anti-detection cooldown: pausing for {cooldown // 60} min {cooldown % 60} sec after {easy_applied_count} Easy Apply submissions...\n")
                        sleep(cooldown)
                ...
                try:
                    ...
                    time.sleep(3)  # Wait for next page jobs to load
...
def _driver_get(url: str, label: str = "", settle: float = 1.5) -> bool:
    '''
    Safe wrapper around driver.get() for pageLoadStrategy="none".
    With "none" strategy, driver.get() returns immediately after the browser
    starts the navigation — it will almost never raise TimeoutException.
    The settle sleep gives the browser a moment to begin rendering before
    callers start looking for elements.
    ...
    try:
        ...
        print_lg(f"[Warning]{tag} driver.get() timed out for: {url[:80]} — continuing.")
        return False
    except Exception as e:
        print_lg(f"[Warning]{tag} driver.get() error: {e}")
        return False
...
def _check_proxy_connectivity() -> bool:
    ...
    try:
        _driver_get("https://www.linkedin.com/", "connectivity-check", settle=1.0)
        # Wait up to 20s for any LinkedIn-specific element — proves the page loaded
        try:
            WebDriverWait(driver, 20).until(
                EC.presence_of_element_located((By.XPATH,
                    "//*[contains(@class,'nav__logo') or "
                    "contains(@class,'global-nav') or "
                    "contains(@class,'main-header') or "
                    "@id='main-content' or "
                    "contains(@class,'sign-in-form')]"
                ))
            )
            ...
            pass
        # Fallback: check it's not a Chrome error page
        if _is_chrome_error_page():
...
def main() -> None:
    ...
    try:
        ...
        if tabs_count >= 10:
            print_lg("NOTE: More than 10 tabs open. Please close some tabs before next run.")
        ##> ------ Yang Li : MARKYangL - Feature ------
        if use_AI and aiClient:
            try:
                if ai_provider.lower() == "openai":
